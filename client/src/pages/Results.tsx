...
import { ArrowLeft, Download, Star, AlertTriangle, Trophy } from 'lucide-react';
...
export default function Results() {
  ...
  const processedResults = resultsData?.results ? (() => {
  ...
  const availableTags = Array.from(new Set(
    resultsData?.results.flatMap(result => result.tags || []) || []
  ));
      ...
      <div className="max-w-md mx-auto bg-white min-h-screen shadow-lg">
        ...
        <div className="p-4 space-y-3">
          {processedResults.length > 0 ? processedResults.map((result, index) => (
            <Card key={result.id} className={getCardClassName(index, result)}>
              <CardContent className="p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="w-12 h-12 bg-gray-200 rounded-full overflow-hidden flex-shrink-0">
                    {result.photoPath ? (
                      <img
                        src={`/objects/${result.photoPath.replace('/objects/', '')}`}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4 mt-1">
                      ...
                      {result.favoriteCount > 0 && (
                        <span className="text-yellow-600 text-sm" data-testid={`result-favorites-${result.id}`}>
